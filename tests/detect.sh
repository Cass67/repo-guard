...
grep -Fq "detected repo languages: python, swift, javascript, typescript, ansible" /tmp/repo-guard-detect.out
grep -Fqx "# repo-guard:python:start" "$target_repo/.pre-commit-config.yaml"
grep -Fqx "# repo-guard:swift:start" "$target_repo/.pre-commit-config.yaml"
